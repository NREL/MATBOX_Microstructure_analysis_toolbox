function [RVE] = Function_nestedtable2(RVE,k_RVE,RVEparameters,number_phase,propertyname,Result_nestedRVE,Sub_folder_RVE,opt,infovol,p,dimension)
...
if dimension == 3
    str_table={['FOV cubic root ' infovol.unit], []};
    length_table = {'cubic root',[]};
else
    str_table={['FOV square root ' infovol.unit], []};
    length_table = {'square root',[]};
end
if strcmp(RVEparameters.type,'C') || strcmp(RVEparameters.type,'H')
    if dimension == 3
        str_table(2)={['FOV square root ' infovol.unit]};
...
if opt.save.xls
    if strcmp(RVEparameters.type,'A') || strcmp(RVEparameters.type,'B') || strcmp(RVEparameters.type,'C') || strcmp(RVEparameters.type,'D') 
        filename = [propertyname '_RVE_FOVconv']; % Filename without extension
    else
