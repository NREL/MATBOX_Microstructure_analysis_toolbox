function [Property_subdomains_statistics, Size_RVE, derivative_convergence, relativedifference_convergence, Size_convergence] = Function_subdomains_statistical_analysis(number_group_size,number_phase_todo,GROUP_SUBDOMAIN,Property_eachsubdomain,voxel_size,RVEparameters)
...
if strcmp(RVEparameters.type,'A') || strcmp(RVEparameters.type,'B') || strcmp(RVEparameters.type,'C') || strcmp(RVEparameters.type,'D')
    ...
    for k_size=1:1:2
        for k_threshold=1:1:n_threshold
            for current_phase_todo=1:1:number_phase_todo
                ...
                idx = find(tmp(:,4)<RVEparameters.threshold_numbersubvolumes);
                n_remove = length(idx);
                tmp(idx,:)=[];
    ...
    for current_phase_todo=1:1:number_phase_todo
        for k=2:1:(number_group_size-1)
            x1 = Property_subdomains_statistics(k-1,1,current_phase_todo);
            x2 = Property_subdomains_statistics(k+1,1,current_phase_todo);            
    ...
    if strcmp(RVEparameters.type,'H')
        for current_phase_todo=1:1:number_phase_todo
            for k=2:1:(number_group_size-1)
                x1 = Property_subdomains_statistics(k-1,2,current_phase_todo);
                x2 = Property_subdomains_statistics(k+1,2,current_phase_todo);
                y1 = Property_subdomains_statistics(k-1,5,current_phase_todo);
                y2 = Property_subdomains_statistics(k+1,5,current_phase_todo);
                derivative_convergence(k-1,current_phase_todo+1,2) = abs((y2-y1)/(x2-x1));
            end
    ...
    relativedifference_convergence(:,1) = Property_subdomains_statistics(:,1,1);
    if strcmp(RVEparameters.type,'G')
        relativedifference_convergence(:,2) = Property_subdomains_statistics(:,3,1);
    end
    if strcmp(RVEparameters.type,'H')
        relativedifference_convergence(:,2) = Property_subdomains_statistics(:,2,1);
    end    
    for current_phase_todo=1:1:number_phase_todo
        ...
        for current_phase_todo=1:1:number_phase_todo
            ...
            if ~isempty(idx)
                ...
                if idx(end)+1 == number_group_size
                    Size_convergence(k_threshold,current_phase_todo,3,1)= Property_subdomains_statistics(end,1,current_phase_todo); % Convergence size is > x
                    if strcmp(RVEparameters.type,'G')
                        Size_convergence(k_threshold,current_phase_todo,3,2)=Property_subdomains_statistics(end,3,current_phase_todo); % Convergence size is > x
                    elseif strcmp(RVEparameters.type,'H')
                        Size_convergence(k_threshold,current_phase_todo,3,2)=Property_subdomains_statistics(end,2,current_phase_todo); % Convergence size is > x
                    end
                else
                ...
                Size_convergence(k_threshold,current_phase_todo,1,1)=Property_subdomains_statistics(1,1,current_phase_todo); % Convergence size is < x
                if strcmp(RVEparameters.type,'G')
                    Size_convergence(k_threshold,current_phase_todo,1,2)=Property_subdomains_statistics(1,3,current_phase_todo); % Convergence size is < x
                elseif strcmp(RVEparameters.type,'H')
                    Size_convergence(k_threshold,current_phase_todo,1,2)=Property_subdomains_statistics(1,2,current_phase_todo);
                end
            end
        end
    end
