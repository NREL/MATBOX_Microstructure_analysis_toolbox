function [Particle_size] = Function_particle_size_CPSD_Algorithm(binary_phase)
...
while current_distance>sqrt(3)
    ...
    Particle_size ( current_distance_map<(current_distance+tolerance) & Particle_size<(current_distance+tolerance) ) = current_distance;
    % Update distance transform
    distance_transform( current_distance_map<(current_distance+tolerance) & Particle_size<(current_distance+tolerance) )=-1;
...
% Apply corrections
% We first grab al the index, and then apply the changes, so that we assure
