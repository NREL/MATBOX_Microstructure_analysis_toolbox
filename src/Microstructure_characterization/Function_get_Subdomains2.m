function [All_subdomain,GROUP_SUBDOMAIN, Wholevolume] = Function_get_Subdomains2(RVEparameters, Domain_size, voxel_size)
% Determine the bounds of subdomains from a domain.

if length(Domain_size)==2 || Domain_size(3)==1
    dimension = 2;
    Domain_size(3) = 1;
else
    dimension = 3;
...
Wholevolume(1) = (prod(Domain_size))^(1/dimension);
if dimension == 3
    ...
    Wholevolume(3) = Domain_size(1)/Domain_size(2);
    Wholevolume(4) = 1;
    Wholevolume(5) = 0;
end
...
if strcmp(RVEparameters.type,'A') % 'Independant subvolumes of same size + keep initial aspect ratio (A)'
    ...
    if RVEparameters.donotcut_smaller
        max_division = floor((initial_volume_um/(RVEparameters.donotcut_smaller_than^dimension))^(1/dimension));
    ...
    if ~isempty(divisions)
        for k=1:1:length(divisions)
            ...
            if dimension == 3
                ...
                GROUP_SUBDOMAIN.id(sub_domain_group).Aspectratio = [Domain_size(1)/divisions(k) Domain_size(2)/divisions(k) ] ./ (Domain_size(2)/divisions(k));
                zk = 1;
            end
            tmp_id = [];
...
if  strcmp(RVEparameters.type,'B') % 'Independant subvolumes of same size + user-defined aspect ratio (B)'
    ...
    if dimension==2
        AR_desired = RVEparameters.Aspectratio_RVE / RVEparameters.Aspectratio_RVE(2);
        ...
        AR_desired = RVEparameters.Aspectratio_RVE / RVEparameters.Aspectratio_RVE(3);
        % Search for the largest subvolume that has the desired AR
        l1_n = 1; l1(1) = Domain_size(1)/l1_n;
        l2(1) = AR_desired(2)*l1(1)/AR_desired(1); l2_n = Domain_size(2)/l2(1);
...
if strcmp(RVEparameters.type,'C') % 'Independant subvolumes of same size + one constant length (C)'
    ...
    if strcmp(RVEparameters.Constantdirection,'Direction 1')
        initial_section = Domain_size(2)*Domain_size(3);
    elseif strcmp(RVEparameters.Constantdirection,'Direction 2')
        initial_section = Domain_size(1)*Domain_size(3);
    elseif strcmp(RVEparameters.Constantdirection,'Direction 3')
        initial_section = Domain_size(1)*Domain_size(2);
    end    
    initial_section_um = initial_section*voxel_size^(dimension-1);
    if RVEparameters.donotcut_smaller
        max_division = floor( (initial_section_um/(RVEparameters.donotcut_smaller_than^(dimension-1)))^(1/(dimension-1)) );
        divisions(divisions>max_division) = [];
    end
    ...
    if ~isempty(divisions)
        for k=1:1:length(divisions)
            ...
            if strcmp(RVEparameters.Constantdirection,'Direction 1')
                Wholevolume(2) = (Domain_size(2) * Domain_size(3))^(1/(dimension-1));
                if dimension == 3
                    ...
                    GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_cubic_length = 0;
                    GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_square_length = (Domain_size(1) * Domain_size(2)/divisions(k))^(1/2);
                    GROUP_SUBDOMAIN.id(sub_domain_group).length = Domain_size(2)/divisions(k);
                    GROUP_SUBDOMAIN.id(sub_domain_group).Aspectratio = [Domain_size(1) Domain_size(2)/divisions(k)] ./ (Domain_size(2)/divisions(k));
                    zk = 1;
                end
                tmp_id = [];
                for y_=1:1:divisions(k)
            ...
            elseif strcmp(RVEparameters.Constantdirection,'Direction 2')
                Wholevolume(2) = (Domain_size(1) * Domain_size(3))^(1/(dimension-1));
                if dimension == 3
                    ...
                    GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_cubic_length = 0;
                    GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_square_length = (Domain_size(1)/divisions(k) * Domain_size(2))^(1/2);
                    GROUP_SUBDOMAIN.id(sub_domain_group).length = Domain_size(1)/divisions(k);
                    GROUP_SUBDOMAIN.id(sub_domain_group).Aspectratio = [Domain_size(1)/divisions(k) Domain_size(2)/divisions(k)] ./ (Domain_size(2));
                    zk = 1;
                end
                tmp_id = [];
                for x_=1:1:divisions(k)
    ...
    if RVEparameters.subs2
        ...
        if RVEparameters.donotcut_smaller
            if (initial_section_um/2)^(1/(dimension-1)) > RVEparameters.donotcut_smaller_than
                ...
                if dimension == 3
                    ...
                    if sum(divisions==2)==0
                        ...
                        if strcmp(RVEparameters.Constantdirection,'Direction 1')
                            GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_cubic_length = 0;
                            GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_square_length = (Domain_size(1) * Domain_size(2)/2)^(1/2);
                            GROUP_SUBDOMAIN.id(sub_domain_group).length = Domain_size(2)/2;
                            GROUP_SUBDOMAIN.id(sub_domain_group).Aspectratio = [Domain_size(1) Domain_size(2)/2] ./ (Domain_size(2)/2);
                            xx_start(1) = 1; xx_end(1) = Domain_size(1);
                            xx_start(2) = 1; xx_end(2) = Domain_size(1);
                            yy_start(1)=1; yy_end(1)=floor(Domain_size(2)/2);
                            yy_start(2)=yy_end(1)+1; yy_end(2)=Domain_size(2);
                        ...
                        elseif strcmp(RVEparameters.Constantdirection,'Direction 2')
                            GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_cubic_length = 0;
                            GROUP_SUBDOMAIN.id(sub_domain_group).equivalent_square_length = (Domain_size(1)/2 * Domain_size(2))^(1/2);
                            GROUP_SUBDOMAIN.id(sub_domain_group).length = Domain_size(1)/2;
                            GROUP_SUBDOMAIN.id(sub_domain_group).Aspectratio = [Domain_size(1)/2 Domain_size(2)] ./ (Domain_size(2));
                            xx_start(1)=1; xx_end(1)=floor(Domain_size(1)/2);
                            xx_start(2)=xx_end(1)+1; yy_end(2)=Domain_size(1);
                            yy_start(1) = 1; yy_end(1) = Domain_size(2);
                            yy_start(2) = 1; yy_end(2) = Domain_size(2);
                            zz_start(1)=1; zz_end(1)=1;
                            zz_start(2)=1; zz_end(2)=1;
...
if strcmp(RVEparameters.type,'D') % 'Independant subvolumes of same size + two constant lengths (D)'
    ...
    if strcmp(RVEparameters.Constantdirection,'Directions 1 and 2')
        initial_length = Domain_size(3);
    elseif strcmp(RVEparameters.Constantdirection,'Direction 1 and 3')
        initial_length = Domain_size(2);
    elseif strcmp(RVEparameters.Constantdirection,'Direction 2 and 3')
        initial_length = Domain_size(1);
    end      
    initial_length_um = initial_length*voxel_size;
    if RVEparameters.donotcut_smaller
        max_division = floor( initial_length_um/RVEparameters.donotcut_smaller_than );
        divisions(divisions>max_division) = [];
    end
    ...
    if ~isempty(divisions)
        for k=1:1:length(divisions)
            ...
            elseif strcmp(RVEparameters.Constantdirection,'Directions 1 and 3')
            ...
            elseif strcmp(RVEparameters.Constantdirection,'Directions 2 and 3')
...
if strcmp(RVEparameters.type,'E') || strcmp(RVEparameters.type,'F') % 'One subvolume + growing from volume center'
    ...
    if strcmp(RVEparameters.type,'E') % Keep initial aspect ratio
        d = 1/(RVEparameters.firstuniquevolume_size/100)^(1/dimension);
    ...
    elseif strcmp(RVEparameters.type,'F') % Custom aspect ratio
        ...
        k = (v1/prod(RVEparameters.Aspectratio_Onesub))^(1/dimension);
        center_volume_size = k*RVEparameters.Aspectratio_Onesub;
    end
    ...
    while 1
        if sub_domain_id>0
            ...
            if strcmp(RVEparameters.Growthrelativeto,'% of the previous subvolume')
                ...
                dv = v0*g;
            elseif strcmp(RVEparameters.Growthrelativeto,'% of the initial volume')
...
if strcmp(RVEparameters.type,'H') % 'One subvolume + growing from volume edge (G)'
    Constantdirection = RVEparameters.Growthdirection;
    d = (RVEparameters.firstuniquevolume_size/100)^(1/(dimension-1));
    ...
    elseif strcmp(Constantdirection,'Direction 3')
        Wholevolume(2) = (Domain_size(1) * Domain_size(2))^(1/(dimension-1));
        x0 = round(Domain_size(1)/2 - d/2*Domain_size(1)); x1 = round(Domain_size(1)/2 + d/2*Domain_size(1));
        y0 = round(Domain_size(2)/2 - d/2*Domain_size(2)); y1 = round(Domain_size(2)/2 + d/2*Domain_size(2));
    ...
    while 1
        if sub_domain_id>0
            ...
            v0 = (x1-x0+1)*(y1-y0+1)*(z1-z0+1); % Current volume
            v1 = v0+dv; % New volume
            d = (v1/prod(Domain_size))^(1/(dimension-1));
            if strcmp(Constantdirection,'Direction 1')
                x0 = 1; x1 = Domain_size(1);
                y0 = round(Domain_size(2)/2 - d/2*Domain_size(2)); y1 = round(Domain_size(2)/2 + d/2*Domain_size(2));
                z0 = round(Domain_size(3)/2 - d/2*Domain_size(3)); z1 = round(Domain_size(3)/2 + d/2*Domain_size(3));
