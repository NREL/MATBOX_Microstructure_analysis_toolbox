function [] = Function_create_figures_nestedRVE(p)
...
if strcmp(p.RVE.type,'A') || strcmp(p.RVE.type,'B') || strcmp(p.RVE.type,'C') || strcmp(p.RVE.type,'D')
    Fig.Name= ['Representative volume element nested analysis: ' p.propertyname];
else
    Fig.Name= ['Convergence nested analysis:' p.propertyname];
end

if strcmp(p.RVE.type,'A') || strcmp(p.RVE.type,'B') || strcmp(p.RVE.type,'E') || strcmp(p.RVE.type,'F')
    str_title = {Fig.Name,p.RVE.name,['Aspect ratio: ' num2str(p.RVE.Aspectratio)]};
elseif strcmp(p.RVE.type,'C') || strcmp(p.RVE.type,'D')
    str_title = {Fig.Name,p.RVE.name};
elseif strcmp(p.RVE.type,'G')
    str_title = {Fig.Name,p.RVE.name,p.RVE.Growthdirection};
elseif strcmp(p.RVE.type,'H')
    str_title = {Fig.Name,p.RVE.name,p.RVE.Constantdirection};    
end   

str_xlabel={['Equivalent length: FOV cubic root ' axisunit], []};
if strcmp(p.RVE.type,'C') || strcmp(p.RVE.type,'H')
    str_xlabel(2)={['Equivalent length: FOV square root ' axisunit]};
    ky = 2;
...
elseif strcmp(p.RVE.type,'C')
    str_ylabel={['Equivalent length: RVE, cubic root ' axisunit], ['Equivalent length: RVE, square root ' axisunit]};
elseif strcmp(p.RVE.type,'D')
    str_ylabel={['Equivalent length: RVE, cubic root ' axisunit], ['RVE, length ' axisunit]};
elseif strcmp(p.RVE.type,'E') || strcmp(p.RVE.type,'F')
    str_ylabel={['Equivalent length: Convergence volume, cubic root ' axisunit], []};
elseif strcmp(p.RVE.type,'G')
    str_ylabel={['Equivalent length: Convergence volume, cubic root ' axisunit], ['Equivalent length: Convergence volume, length ' axisunit]};
elseif strcmp(p.RVE.type,'H')
    str_ylabel={['Equivalent length: Convergence volume, cubic root ' axisunit], ['Equivalent length: Convergence volume, square root ' axisunit]};    
end
