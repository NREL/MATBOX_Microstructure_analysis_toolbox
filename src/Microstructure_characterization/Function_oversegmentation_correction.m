function [Labelmap] = Function_oversegmentation_correction(BW, Labelmap, sizethreshold, cpsd_refining, verbose)

% STEP 1: search for non-continguous discrete particle and correct them
% STEP 2: Size filter particle
...
% Checksum (hash) on Labelmap is used to verify that the iterative process does not bring us back to a previous state and run indefinitely
...
% Calculate the checksum on the 3d volume can lead to a OUT OF MEMORY error. To avoid this, we calculate it slice per slice
Checksum_state0_Labelmap=[];
for z=1:1:Domain_size(3)
    current_checksum = DataHash(Labelmap(:,:,z));
    Checksum_state0_Labelmap=[Checksum_state0_Labelmap current_checksum];
end
Checksum_list.state(1).hash=Checksum_state0_Labelmap;

iteration_number=0;
number_change=1e9;
if verbose
    fprintf ('Oversegmentation correction.\n');
end
while number_change~=0
    ...
    unique_particle(unique_particle==0)=[]; % Remove complementary phase
    ...
    for current_=1:1:number_id
        ...
        binary_particle(Labelmap==current_id)=1;
        % Label particle
        L_particle = bwlabeln(binary_particle,6);
        % Unique label
        unique_cluster = unique(L_particle);
        unique_cluster(unique_cluster==0)=[]; % remobe background cluster id
        ...
        if cluster_number>1
            ...
            for analysed_cluster=2:1:cluster_number
                ...
                for current_voxel=1:1:number_voxel_cluster
                    ...
                    % Check x-
                    if x_>1
                        if Labelmap(x_-1,y_,z_)~=current_id
                            surrounding_values=[surrounding_values Labelmap(x_-1,y_,z_)];
                            if Labelmap(x_-1,y_,z_)~=0
                                pos_surrounding=[pos_surrounding; [x_-1,y_,z_,0,Labelmap(x_-1,y_,z_)]];
                            end
                        end
                    end
                    % Check y-
                    if y_>1
                        if Labelmap(x_,y_-1,z_)~=current_id
                            surrounding_values=[surrounding_values Labelmap(x_,y_-1,z_)];
                            if Labelmap(x_,y_-1,z_)~=0
                                pos_surrounding=[pos_surrounding; [x_,y_-1,z_,0,Labelmap(x_,y_-1,z_)]];
                            end
                        end
                    end
                    % Check z-
                    if z_>1
                        if Labelmap(x_,y_,z_-1)~=current_id
                            surrounding_values=[surrounding_values Labelmap(x_,y_,z_-1)];
                            if Labelmap(x_,y_,z_-1)~=0
                                pos_surrounding=[pos_surrounding; [x_,y_,z_-1,0,Labelmap(x_,y_,z_-1)]];
                            end
                        end
                    end
                    % Check x+
                ...
                if length(unique(surrounding_values))==1 && surrounding_values(1)==0
                    ...
                    for current_voxel=1:1:number_voxel_cluster
                        ...
                        % Replace value
                        Labelmap(x_,y_,z_)=preominent_value;
                    end
                end
    ...
    if sizethreshold>0
        ...
        for current_=1:1:number_id
            ...
            current_id = unique_particle(current_);
            % Size
            D_PSD_particle_size = sum(sum(sum(Labelmap==current_id)));
            if D_PSD_particle_size<=sizethreshold
                index_particle=find(Labelmap==current_id);
                % Get all their coordinate
        ...
        for current_id=2:1:number_id % 0 is the complementary phase
            ...
            index_ = find(Labelmap==id_to_be_replaced);
            Labelmap(index_)=current_id-1;
        end
        % Get all discrete particle id
        unique_particle = unique(Labelmap);
        unique_particle(1)=[]; % Remove the 0, allocated to the complementary phase
    ...
    if cpsd_refining
        ...
        if iteration_number==1
            ...
            [C_PSD_particle_size] = Function_particle_size_CPSD_Algorithm(BW);
        end    
        ...
        for current_=1:1:zone_number
            ...
            % Get all voxels
            index_zone = find(Labelmap==current_zone);
            % Get all coordinates
    ...
    if iteration_number>1
        number_change=sum(sum(sum(previous_discrete_particle~=Labelmap)));
        if verbose
            fprintf ('   CONVERGENCE CHECK: Number of voxels changed compared with previous iteration: %i\n',number_change);
        end
    end
    % For the next iteration
    previous_discrete_particle = Labelmap;
    
    %% CHECKSUM
    % Current Discrete_particle checksum
    Checksum_currentstate_Labelmap=[];
    for current_=1:1:Domain_size(3)
        current_checksum = DataHash(Labelmap(:,:,current_));
        Checksum_currentstate_Labelmap=[Checksum_currentstate_Labelmap current_checksum];
    end
    
    % Compare with previous checksum
    exit_loop_checksum=0;
    for previous_state=1:1:iteration_number
        Checksum_previousstate_Label_lake = Checksum_list.state(previous_state).hash;
        check_checksum = isequal(Checksum_previousstate_Label_lake,Checksum_currentstate_Labelmap);
        if check_checksum==1
            exit_loop_checksum=1;
        end
    end
    % Save current checksum in the list
    Checksum_list.state(iteration_number+1).hash=Checksum_currentstate_Labelmap;
