function [] = Function_Discrete_particle_size_watershed_immersion(Phase_microstructure, PROPERTY, OPTIONS, INFO, foo, foo2)
%Identify particles with a watershed immersion approach and calculate their morphology and size 
% Function_Discrete_particle_size_watershed_immersion(array, PROPERTY, OPTIONS, INFO) - when use with the toolbox
% or
% Function_Discrete_particle_size_watershed_immersion(array, voxelsize, customfunction, cpsd_refining, details_convergence, visualize_2D) - when use as a standalone function
...
if nargin ~= expected_number_argument % Unexpected number of argument
    ...
    if nargin == 6 % Case for function called as: Function_Discrete_particle_size_watershed_immersion(Phase_microstructure, voxel_size, customfunction, cpsd_refining, details_convergence, visualize_2D)
        voxel_size = PROPERTY;
        customfunction = OPTIONS;
        cpsd_refining = INFO;
        details_convergence = foo;
        visualize_2D = foo2;
        ...
        % Parameters
        PROPERTY.particlesize_watershed.customfunction = customfunction;
        PROPERTY.particlesize_watershed.cpsd_refining = cpsd_refining;
        PROPERTY.particlesize_watershed.details_convergence = details_convergence;        
        PROPERTY.particlesize_watershed.visualize_2D = visualize_2D; 
        
    else % Incorrect number of argument
        disp 'Error calling Function_Discrete_particle_size_watershed_immersion. Wrong number of argument.'
        help Function_Discrete_particle_size_watershed_immersion
    end
end
...
if ispc
    ...
    Sub_folder = 'Particle_size_watershed\'; % Result are saved in this subfolder
else
    main_folder = [OPTIONS.mainsavefolder '/'];
    Sub_folder = 'Particle_size_watershed/'; % Result are saved in this subfolder
end
...
if min(Domain_size) == 1 % 2D case
    ...
    PROPERTY.particlesize_watershed.visualize_2D = false;
end
...
if OPTIONS.displaytext==true
    disp '    PARTICLE SIZE - WATERSHED IMMERSION (D-PSD) METHOD';
    disp '    -------------------------------------------------------------------';
    disp ' ';
end
...
%% PARAMETERS
customfunction = PROPERTY.particlesize_watershed.customfunction;
cpsd_refining = PROPERTY.particlesize_watershed.cpsd_refining;
details_convergence = PROPERTY.particlesize_watershed.details_convergence;
visualize_2D = PROPERTY.particlesize_watershed.visualize_2D;
...
for current_phase=1:1:number_phase % Loop over all phases
    ...
    [Particle_size,Particle_label_tmp] = Function_Discrete_particle_size_watershed_immersion_algorithm(binary_phase,cpsd_refining,customfunction,details_convergence,visualize_2D);
    ...
    results_correlation(current_phase).Particle_diameter_mean_Watershed = PSD_results(current_phase,2);
    results_correlation(current_phase).Particle_diameter_std_Watershed = PSD_results(current_phase,4);
    results_correlation(current_phase).Particle_diameter_relstd_Watershed = PSD_results(current_phase,5);
    results_visualization(current_phase).Particle_diameter_Watershed = Particle_size;
    results_visualization(current_phase).Particle_ParticleLabel_Watershed = Particle_label;
end
...
Results_watershed.Table_time = Table_time; % Save in main table result

% Result calculated on whole volume
Table_watershed_size = table(INFO.phasename,PSD_results(:,1),PSD_results(:,2),PSD_results(:,3),PSD_results(:,4),PSD_results(:,5),...
    'VariableNames',{'Name' 'Min' 'Mean' 'Max' 'Std' 'Std_percents'});
Table_watershed_d50 = table(INFO.phasename,PSD_results(:,6),PSD_results(:,7),PSD_results(:,8),PSD_results(:,9),...
    'VariableNames',{'Name' 'd50' 'Smoothed_d50' 'Distribution_integral' 'Smoothed_distribution_integral'});
Results_watershed.Table_watershed_size = Table_watershed_size; % Save in main table result
Results_watershed.Table_watershed_d50 = Table_watershed_d50;
...
Results_watershed.Table_cumulative_sizedistribution = Table_cumulative_sizedistribution; % Save in main table result

%% SAVE TABLES
if OPTIONS.save_xls==true
    filename = 'Watershed_PSD'; % Filename without extension
    % Prepare the data
    clear DATA_writetable
    DATA_writetable.sheet(1).name='Results';
    DATA_writetable.sheet(1).table=Table_watershed_size;
    DATA_writetable.sheet(2).name='D50_from_cumulativefct';
    DATA_writetable.sheet(2).table=Table_watershed_d50;       
...
if OPTIONS.displaytext==true
    ...
    disp(Table_watershed_size)
    disp 'Particle diameter (from cumulative function)';
    disp(Table_watershed_d50)
...
for current_phase=1:1:number_phase % Loop over all phases
    ...
    parameters_distributionfigure.filename = ['Diameter_watershed_' INFO.phase(current_phase).name];
    ...
    parameters_distributionfigure.title = ['Particle diameter (watershed), ' INFO.phase(current_phase).name];
...
alongdirection_parameters.field = 'Particle_diameter_Watershed';
...
alongdirection_parameters.Table_filename = 'Diameter_watershed';
alongdirection_parameters.figure_name = 'Diameter_watershed';
alongdirection_parameters.axe_title = 'Particle diameter (watershed)';
alongdirection_parameters.figure_title = 'Particle diameter along directions (watershed)';
alongdirection_parameters.figure_filename = 'Diameter_watershed_along_directions';
...
Results_watershed.Table_evolution = Table_evolution; % Save in main table result
...
for current_phase=1:1:number_phase % Loop over phases
    data_Particle_size = results_visualization(current_phase).Particle_diameter_Watershed;
    data_Particle_label = results_visualization(current_phase).Particle_ParticleLabel_Watershed;
...
%             end
    ...
    sgtitle(Fig,['Particle diameter, label and watershed line (watershed), ' INFO.phase(current_phase).name],'FontWeight','bold','FontSize',OPTIONS.Fontsize_title+2,'FontName',OPTIONS.fontname);
    if OPTIONS.save_fig == true % Save figure
        filename= sprintf('View_watershed_%s', INFO.phase(current_phase).filename);
...
for current_phase=1:1:number_phase
    ...
    Particle_size = results_visualization(current_phase).Particle_diameter_Watershed; % Size
    Particle_label = results_visualization(current_phase).Particle_ParticleLabel_Watershed; % Label
    ...
    % particle sphericity
    [results_correlation, results_visualization, Table_discrete_particlemorpholgy_analysis, unique_particle, mass_center_particle] = Function_analyse_discrete_particle_morphology(Particle_size, Particle_label, results_correlation, results_visualization, current_phase, Current_folder, INFO, OPTIONS, 'watershed');
    Results_watershed.Phase(current_phase).Table_discrete_particlemorpholgy_analysis = Table_discrete_particlemorpholgy_analysis;
    ...
    Results_watershed.Phase(current_phase).Table_discrete_particleinterface_analysis = Table_discrete_particleinterface_analysis;
    ...
    Results_watershed.Phase(current_phase).Table_discrete_particle_graphnetowrk = Table_discrete_particle_graphnetowrk;
...
filename = 'Diameter_watershed_calculation_time';
[Results_watershed] = function_time_figure(Time_measure,date_start, date_end, Results_watershed, Current_folder, filename, 'Particle size (watershed)', OPTIONS);
...
if OPTIONS.save_resultsmat == true
    ...
    save([Current_folder 'Results_particlediameter_watershed'],'Results_watershed')
    Sub_folder = 'Correlation\'; % Result are saved in this subfolder
    Current_folder = [main_folder Sub_folder];
    if exist(Current_folder,'dir')==0 % Folder existence is checked, and created if necessary
        mkdir(Current_folder);
    end
    save([Current_folder 'Correlation_particlediameter_watershed'],'results_correlation')
    Sub_folder = 'Visualization\'; % Result are saved in this subfolder
    Current_folder = [main_folder Sub_folder];
    if exist(Current_folder,'dir')==0 % Folder existence is checked, and created if necessary
        mkdir(Current_folder);
    end
    save([Current_folder 'Visualization_particlediameter_watershed'],'results_visualization','-v7.3');    
end
