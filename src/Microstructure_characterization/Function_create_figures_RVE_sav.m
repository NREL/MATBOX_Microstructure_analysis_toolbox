        ...
        if p.todo(current_phase)
            ...
            for id_axe=1:1:n_axe % Iterate over axe
                ...
                if id_axe==1 ||  id_axe==4
                    ...
                    % - Legend
                    h_legend = legend(sub_axes,'Mean +/- standard deviation','Extremums','5% interval confidence',['Whole volume, ' num2str(p.Wholevolume_size(kx),'%1.1f') ' ' lgdunit],'Location','best');
...
elseif strcmp(p.RVE.type,'D') || strcmp(p.RVE.type,'E') || strcmp(p.RVE.type,'F')
   
    if strcmp(p.RVE.type,'F')
