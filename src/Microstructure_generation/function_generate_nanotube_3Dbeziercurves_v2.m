function [tube_phase, tube_id, tube_skeleton, tubes, outcome] = function_generate_nanotube_3Dbeziercurves_v2(domain_size,Bezier_pars,sav,stopcond,cropp,p)
...
tube_can_overlap_with_tube = false;
tube_can_overlap_with_mask = false;
...
elevations = [];
force_seedequidistribution = true;
...
if p.seeds_onlyatedges && force_seedequidistribution
    seed_xyz = [0 0 0];
end
...
while k_tube < stopcond.n_tube && volume_fraction < stopcond.target_volumefraction
    ...
    % Select seed
    if p.seeds_onlyatedges && k_tube>0 && force_seedequidistribution
    ...
    if p.maxdeviation_azimuthchange==0 && p.mean_azimuthchange ==0 && p.maxdeviation_elevationchange==0 && p.mean_elevationchange==0 % Straight tube, simplified code
        ...
        while true % Exit only through break statment
            ...
            if min(Q(:,1)) >= 1 && min(Q(:,2)) >= 1 && min(Q(:,3)) >= 1 && max(Q(:,1)) <= domain_size(1) && max(Q(:,2)) <= domain_size(3) && max(Q(:,3)) <= domain_size(3) % Tube is within bounds
