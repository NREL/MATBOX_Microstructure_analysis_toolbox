function [microstructure3D, phase, particle_data, outcome, overlapping_stat] = function_generate_ellipsoid_microstructure(domain_size,phase,tolerance_vf,forceodddiameter,overlapping,check_contiguity, stoping_conditions, do_verification, save_options)
% Generate ellipsoids-based microstructrures
% Francois Usseglio-Viretta, NREL

acc.id_selector_behavior = 'random';
%acc.id_selector_behavior = 'accelerated';
acc.n_fail_1 = 5000;
acc.n_fail_2 = 1000;
acc.idxfirstchoice = 'random';
%acc.idxsecondchoice = 'random';
acc.idxsecondchoice = 'dmap';
%acc.idxsecondchoice = 'z';
%acc.idxsecondchoice = 'dmap and z';
acc.use_dmapapproximation = false;
...
%% PARAMETERS
simulate_calendering = false; % Useful to set it true to achieve high density (Ignore it and keep it false it is outdated)
        ...
        while phase_volume_filled(current_phase,1) <= phase_volume_target && stop_condition_not_reached
            ...
            if new_particle_hasbeen_generated
                ...
                if strcmp(acc.id_selector_behavior,'accelerated') && (strcmp(acc.idxsecondchoice,'dmap') || strcmp(acc.idxsecondchoice,'dmap and z')) && acc.use_dmapapproximation
                    if particle_id==1
                        dmap = bwdist(microstructure3D.phase);
                    else
                        lastparticle = particle_data(particle_id,:);
                        rmax = max(lastparticle(6:8));
                        x0 = max([round(lastparticle(3)-dmap_updaterange*rmax) 1]);
                        x1 = min([round(lastparticle(3)+dmap_updaterange*rmax) domain_size(1)]);       
                        y0 = max([round(lastparticle(4)-dmap_updaterange*rmax) 1]);
                        y1 = min([round(lastparticle(4)+dmap_updaterange*rmax) domain_size(2)]);     
                        z0 = max([round(lastparticle(5)-dmap_updaterange*rmax) 1]);
                        z1 = min([round(lastparticle(5)+dmap_updaterange*rmax) domain_size(3)]);
                            ...
                            for k_other = 1:1:n_otherparticles % Loop over all existing particles
                                ...
                                if length(tmp)>=1
                                    ...
                                    if max(ratio_overlapping, ratio_overlapping_other)<=current_maximum_interpenetration % Check overlapping
                                        ...
                                        if ratio_other <= overlapping.minvolume(other_phasenumber) - tol_overlaping(current_phase) || ratio_ <= overlapping.minvolume(current_phase) - tol_overlaping(current_phase);
                                            new_particle_hasbeen_generated = false;  % Not necessary. For clarification sake.
                                            continue_whileloop_and_go_to_next_randcenter = true;
                                            break
                                        else
