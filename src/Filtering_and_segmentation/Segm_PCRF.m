function [Label_lake] = Segm_PCRF(BW, p)
...
p.percluster = 1; % No reason not to use it

%% ANALYSE PER CLUSTER
if p.percluster
    if dimension==2
        Clusters = bwlabel(BW,4);
    else
        Clusters = bwlabeln(BW,6);
    end
...
for kcluster = 1:1:ncluster
    ...
    if p.percluster
        ...
        if dimension==2
            ...
            BW = Clusters(x_mincluster:x_maxcluster,y_mincluster:y_maxcluster);
        else
            z_mincluster = min(IZ); z_maxcluster = max(IZ);
            BW = Clusters(x_mincluster:x_maxcluster,y_mincluster:y_maxcluster,z_mincluster:z_maxcluster);
        end
        BW(BW~=uni_clusters(kcluster))=0;
        BW(BW~=0)=1;
    end
    ...
    % Number of voxel
    number_voxel_phase = sum(sum(sum(BW==1)));
    % Find index for the phase
    index_phase = find(BW==1);
    % Get all coordinates
    [P1,P2,P3] = ind2sub(Domain_size,index_phase);

    % % For the complementary phase
    % Number of voxel
    number_voxel_complementary = sum(sum(sum(BW==0)));
    % Find index for the phase
    index_complementary_phase = find(BW==0);
    % Get all coordinates
    ...
    number_voxel_border = sum(sum(sum(BW==2)));
    % Find index for the border voxel
    index_border_phase = find(BW==2);
    % Get all coordinates
    ...
    % Initialise
    history_test=zeros(2*p.max_dist,3);
    ...
    for current_voxel_border=1:1:number_voxel_border
        ...
        x_min = max(1,x_b-p.max_dist); y_min = max(1,y_b-p.max_dist); z_min = max(1,z_b-p.max_dist);
        x_max = min(Domain_size(1),x_b+p.max_dist); y_max = min(Domain_size(2),y_b+p.max_dist); z_max = min(Domain_size(3),z_b+p.max_dist);
        ...
        for current_voxel_phase=1:1:number_voxel_close
            ...
            if index_array(x_pp,y_pp,z_pp,2)~=current_voxel_border
                ...
                while (test_==1 && (iteration_test+1)<=data_(current_voxel_phase,5))
                    ...
                    % Check voxel
                    test_ = BW(x_tested,y_tested,z_tested);
                ...
                if test_==1
                    ...
                    force_=1/(data_(current_voxel_phase,1)^p.exponent_law); % total force >0
                    ...
                    for marker_=1:1:data_(current_voxel_phase,5)
                        ...
                        if index_array(x_int, y_int, z_int,2)~=current_voxel_border
                            % Force
                            force_=1/(v1(marker_,1)^p.exponent_law); % total force >0
    ...
    if p.visualize_2D
        ...
        tmp = Repulsive_matrix(:,:,1,1); tmp(BW==0)=NaN;
        ...
        tmp = Repulsive_matrix(:,:,1,2); tmp(BW==0)=NaN;
        ...
        hold(sub_axes_illustration(3).s,'on'); % Active subplot
        tmp = sign(Repulsive_matrix(:,:,1,1)); tmp(BW==0)=0;
        imagesc(sub_axes_illustration(3).s,tmp);
        ...
        tmp = sign(Repulsive_matrix(:,:,1,2)); tmp(BW==0)=0;
    ...
    for current_voxel=1:1:number_voxel_phase
        ...
        if Label_lake(x_,y_,z_)==0
            ...
            while movement==1
                ...
                if p.visualize_2D % Uncomment to see step by step. Although, visualization is very slow.
                    ...
                    %                 tmp = sign(Repulsive_matrix(:,:,1,1)); tmp(BW==0)=0; tmp(Label_lake~=0)=0;
                    ...
                    %                 tmp = sign(Repulsive_matrix(:,:,1,2)); tmp(BW==0)=0; tmp(Label_lake~=0)=0;
    ...
    tmp_BW = zeros(size(BW));
    tmp_BW(BW==1)=1;
    [Label_lake] = Function_correct_DPSD_identification(tmp_BW, Label_lake, p.cpsd_refining, p.details_convergence, chess_pattern_removal, P1, P2, P3, p.visualize_2D);
...
if p.percluster % Remove temporary arrays
