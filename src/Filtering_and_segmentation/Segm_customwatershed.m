function [Instances] = Segm_customwatershed(BW,p)
...
% However, the code in this program is original.

% The oversegmentation correction is from:
...
if p.visualize_2D
    ...
    y_video_l = []; y_video_2 = []; num_voxel = sum(sum(sum(BW==1)));
    ...
    imagesc(sub_axes_illustration(1).s,BW);
...
Distance_map=bwdist(~BW,'euclidean');
...
Distance_map(~BW) = +Inf; % A negative infinite value is attributed to the complementary
...
Instances = zeros(sz(1), sz(2), sz(3)); % Initialisation
if p.details_convergence
...
for current_altitude_iteration = 1:1:number_altitude % We start from the lower position and go up
    ...
    if progress_force_calculation>=next_step_display && p.details_convergence
    ...
    binary_altitude_map = zeros(sz(1), sz(2), sz(3)); % Initialisation
    ...
    if current_altitude_iteration>1
        for current_iteration = 1:1:number_cluster % Loop over all the cluster identified
            ...
            binary_cluster = zeros(sz(1), sz(2), sz(3)); % Create binary cluster
            binary_cluster(Label_current_altitude==Current_label)=1;
            index_cluster = find(binary_cluster==1); % Index voxels of the cluster
            intersection = binary_cluster.*Instances; % Calculate intersection with the already identified lake
            ...
            if number_intersection==0
                ...
                binary_existing_lake = zeros(sz(1), sz(2), sz(3));
                binary_existing_lake(Instances~=0)=1;
                ...
                if min_distance>2
                    ...
                    max_lake_id = max(unique(Instances))+1;
                    % Attribute this new id to the cluster
                    Instances(index_cluster)=max_lake_id;
                else
                    ...
                    id_adjacent_lake = Instances(index_adjacent_lake);
                    ...
                    if number_adjacent_lake==1
                        ...
                        Instances(index_cluster)=id_adjacent_lake(1);
                    else
                        ...
                        for current_voxel=1:1:length(index_cluster)
                            ...
                            [x_tmp,y_tmp,z_tmp] = ind2sub(sz,index_cluster(current_voxel));
                            choose_lake(:,1)=x_tmp;
                            choose_lake(:,2)=y_tmp;
                            choose_lake(:,3)=z_tmp;
                            for current_lake=1:1:number_adjacent_lake
                                [x_tmp,y_tmp,z_tmp] = ind2sub(sz,index_adjacent_lake(current_lake));
                                choose_lake(current_lake,4)=x_tmp;
                                choose_lake(current_lake,5)=y_tmp;
                                choose_lake(current_lake,6)=z_tmp;
                                choose_lake(current_lake,7)=Instances(index_adjacent_lake(current_lake));
                            end
                            % Calculate distance
                            ...
                            Instances(index_cluster(current_voxel))=choose_lake(1,7);
                        end
                    end
                end
            end
            ...
            if number_intersection>0
                ...
                different_lake = unique(Instances(index_intersection));
                ...
                if number_different_lake==1
                    ...
                    Instances(index_cluster)=different_lake;
                end
                ...
                if number_different_lake>1
                    ...
                    % Get all coordinates
                    [C1,C2,C3] = ind2sub(sz,index_cluster);
                    % Min, max coordinates
                    ...
                    subdomain_lake = Instances(x_min:x_max,y_min:y_max,z_min:z_max);
                    ...
                    Instances(x_min:x_max,y_min:y_max,z_min:z_max)=subdomain_lake;
                end
            end
        end
    else
        ...
        Instances = Label_current_altitude;
    end
    ...
    if p.visualize_2D
        ...
        currentvalues = unique(Instances);
        for k=1:1:length(currentvalues)
            idx=find(Instances==currentvalues(k));
        ...
        % Progression
        volumeprogression = 100*sum(sum(sum(Instances~=0)))/num_voxel;
...
if p.visualize_2D
    ...
    % Find watershedlines
    [index_border_label,~,~,~] = Function_identify_labelsedges(Instances, background, edgewithbackground);
...
unique_particle = unique(Instances);
...
[Instances] = Function_correct_DPSD_identification(BW, Instances, p.cpsd_refining, p.details_convergence, chess_pattern_removal, P1, P2, P3, p.visualize_2D);
